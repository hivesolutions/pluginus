...
export class Plugin extends Observable {
    ...
    async triggerAll(event, ...args) {
        ...
        // triggers the base event structure and adds the resulting promise
        // to the list of promises waiting bo be fulfilled
        promise = this.trigger(event, ...args);
        promises.push(promise);

        // triggers the global wide event in the owner (plugin manager) and adds
        // the resulting promise to the list of promises waiting bo be fulfilled
        promise = this.owner.trigger(`plugin:${event}`, this, ...args);
        promises.push(promise);
